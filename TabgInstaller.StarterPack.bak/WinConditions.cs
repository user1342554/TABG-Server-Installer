            ...
            if (field != null)
            {
                ...
                if (state == GameState.Started)
                {
                    ...
                    else if(Config.winCondition == WinCondition.Default)
                    {
                        ...
                        Plugin.Log?.LogInfo("Checking Game State: Teams Alive: " + aliveTeams.ToString());
                        if (aliveTeams <= 1)
                        {
                    ...
                    {
                        Plugin.Log?.LogInfo("Checking Game State: Teams Alive: " + m_GameRoom.CurrentGameStats.GetAliveTeams().ToString());
                    }
                    return false;
                }
            }
